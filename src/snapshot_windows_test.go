...
func TestNativeDocumentationRenders(t *testing.T) {
	...
	if dir == "" {
		t.Skip("optional native documentation export")
	}
	...
	for _, lang := range []string{"zh-CN", "en"} {
		for _, light := range []bool{false, true} {
			...
			options := CardOptions{Locale: a.L, DPI: a.DPI, MaxHeight: 2600, Light: light, HourDetails: true}
			a.applyPreviewOptions(&options, now)
			layout := CompactLayout(a.Data.DayAt(now), a.Data.HourAt(now), options, func(s string, size int, bold bool) int { return a.textWidth(surface.DC, s, size, bold) })
			...
			if layout.Height != layout.ContentHeight {
				t.Fatal("documentation card would be truncated")
			}
			...
			if widget.Pix[3] > 1 {
				t.Fatal("transparent taskbar has a visible background")
			}
	...
	if e = os.WriteFile(filepath.Join(dir, "render-metadata.json"), metadata, 0644); e != nil {
		t.Fatal(e)
	}
